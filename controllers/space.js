...
module.exports = {
  ...
  post: (req, res) => {
    ...
    const args = { url: req.body.url, name: req.body.name, userId: req.session.userid };
  ...
  put: (req, res) => {
    if (!req.session.userid) return res.status(401).send({error: {status: 401, message: "본인 인증을 한 후 스페이스 목록을 수정할 수 있습니다."}});
    if (!req.body.name) return res.status(400).send({error: {status: 400, message: 'body를 다음과 같이 수정해주세요: {name}'}});
    const args = { id: req.params.spaceId, name: req.body.name, userId: req.session.userid };
  ...
  delete: (req, res) => {
    ...
    const args = { id: req.params.spaceId, userId: req.session.userid };
