    ...
    if (!req.session.userid) {
      ...
      userModel.get(args, (err, results) => {
        if (err) res.status(500).send({error: {status: 500, message: "회원정보 가져오기 실패"}});
          ...
          res.status(200).send({email: results[0].email, name: results[0].name, oauth_signup : results[0].oauth_signup});
        }
      });
    }
  },
    ...
    if(!req.session.userid) {
      ...
      userModel.put(args, (err, results) => {
        if (err) res.status(500).send({error: {status: 500, message: "회원정보 수정 실패"}});
          ...
          if(!results.length) return res.status(409).send({error: {status: 409, message: "다른 유저가 사용 중인 email 입니다."}}); 
          else{
